      ...
      <div className="relative aspect-square bg-n-2/10 overflow-hidden">
        ...
        {description && (
          <div className="absolute inset-0 bg-black/60 p-4 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-center backdrop-blur-[2px]">
            <p className="line-clamp-6 font-medium leading-relaxed">
              {description}
            </p>
          </div>
        )}
        ...
        <div className="flex items-end justify-between gap-2 mt-auto">
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline gap-1.5">
              <span className="text-base font-bold text-n-8">
            ...
            {product?.vendor && (
              <Link
                to={`/p/${product.vendor.username}`}
                className="flex items-center gap-1.5 mt-1.5 group/vendor hover:opacity-80 transition-opacity"
              >
                <UserAvatar
                ...
                />
                <span className="text-[11px] font-medium text-n-6 truncate hover:text-primary-3 transition-colors max-w-[100px] xs:max-w-max">
            ...
            {String(authUser?._id) !==
              ...
              !isOutOfStock && (
                <button
                  ...
                  className="w-8 h-8 rounded-full border border-primary-3 text-primary-3 flex items-center justify-center hover:bg-primary-3 hover:text-white transition-all shadow-sm active:scale-95 flex-shrink-0 bg-white"
                  title="Add to Cart"
                >
                  <Plus size={18} />
                </button>
